  ...
  return (
    <Stack spacing={3}>
      <Box>
        ...
        <Typography variant="h5" fontWeight={600}>All Appointments</Typography>
        <Typography color="text.secondary" variant="body2" sx={{ mt: 0.5 }}>
          Overview of all customer home installation appointments across all ISP teams.
        </Typography>
      </Box>
                    ...
                    {appt.status === "Pending" && (
                      <>
                        <Button variant="contained" color="success" size="small" startIcon={<CheckCircleIcon />}
                          onClick={() => setActionTarget({ id: appt._id, status: "Approved" })} fullWidth>
                          Approve
                        </Button>
                        <Button variant="outlined" color="error" size="small" startIcon={<CancelIcon />}
                          onClick={() => setActionTarget({ id: appt._id, status: "Cancelled" })} fullWidth>
                          Cancel
                        </Button>
                      </>
                    )}
                    {appt.status === "Approved" && (
                      <>
                        <Button variant="contained" color="info" size="small" startIcon={<DoneAllIcon />}
                          onClick={() => setActionTarget({ id: appt._id, status: "Completed" })} fullWidth>
                          Mark Complete
                        </Button>
                        <Button variant="outlined" color="error" size="small" startIcon={<CancelIcon />}
                          onClick={() => setActionTarget({ id: appt._id, status: "Cancelled" })} fullWidth>
      ...
      <Dialog open={Boolean(actionTarget)} onClose={() => setActionTarget(null)}>
        ...
        <DialogContent>
          <DialogContentText>
            Are you sure you want to mark this appointment as <strong>{actionTarget?.status}</strong>?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
